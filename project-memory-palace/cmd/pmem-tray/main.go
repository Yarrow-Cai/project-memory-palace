...
// generatePNG produces a 32x32 RGBA icon as PNG bytes.
...
// generateICO wraps the icon PNG in a valid ICO container.
// The getlantern/systray library on Windows calls LoadImage(IMAGE_ICON),
// which only supports .ico files, not raw PNG.
func generateICO() []byte {
	...
	buf[2], buf[3] = 1, 0   // type: ICO (little-endian)
	buf[4], buf[5] = 1, 0   // count: 1 image

	// Directory entry
	buf[6] = 32              // width
	buf[7] = 32              // height
	buf[8] = 0               // color palette count
	buf[9] = 0               // reserved
	buf[10], buf[11] = 1, 0  // planes (LE uint16)
	buf[12], buf[13] = 32, 0 // bpp (LE uint16)
	buf[14], buf[15], buf[16], buf[17] = byte(pngSize), byte(pngSize>>8), byte(pngSize>>16), byte(pngSize>>24)
	buf[18], buf[19], buf[20], buf[21] = 22, 0, 0, 0 // offset = 22 (header 6 + entry 16)
...
func startAPI() {
	...
	fmt.Fprintln(os.Stderr, "API server started on :8147")
