...
type MemoryService struct { projectRoot string; idx *index.MemoryIndex }
func New(projectRoot string) *MemoryService { return &MemoryService{projectRoot: projectRoot, idx: index.NewMemoryIndex(projectRoot)} }
...
func (s *MemoryService) Recall(query string, filters map[string]any, limit int) ([]map[string]any, error) { if err := s.InitProject(); err != nil { return nil, err }; return s.idx.Search(query, filters, limit) }
...
func (s *MemoryService) OpenMemory(memoryID string) (map[string]any, error) {
	...
	for _, card := range cards { if card.ID == memoryID { return cardToMap(card), nil } }
	return nil, &MemoryNotFoundError{ID: memoryID}
}

func (s *MemoryService) ListRecent(limit int) ([]map[string]any, error) { if err := s.InitProject(); err != nil { return nil, err }; return s.idx.Recent(limit) }
...
func (s *MemoryService) UpdateMemory(memoryID string, updates map[string]any) (map[string]any, error) {
	...
	changed := false
	if status, ok := updates["status"].(string); ok { if !memory.MemoryStatuses[status] { return nil, fmt.Errorf("invalid status: %s", status) }; if status != existing["status"] { changed = true; existing["status"] = status } }
	if conf, ok := updates["confidence"]; ok { f, ok := toFloat64(conf); if !ok || f < 0 || f > 1 { return nil, fmt.Errorf("confidence must be 0.0-1.0") }; if f != existing["confidence"] { changed = true; existing["confidence"] = f } }
	if tags, ok := updates["tags"]; ok { l, err := toStringList(tags); if err != nil { return nil, fmt.Errorf("tags: %w", err) }; changed = true; existing["tags"] = l }
	if rels, ok := updates["relations"].(map[string]any); ok { cur, _ := existing["relations"].(map[string][]string); if cur == nil { cur = make(map[string][]string) }; for k, v := range rels { targets := toStringSlice(v); for _, t := range targets { if !has(cur[k], t) { cur[k] = append(cur[k], t); changed = true } } }; existing["relations"] = cur }
...
func has(slice []string, item string) bool { for _, s := range slice { if s == item { return true } }; return false }
